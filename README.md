...
* **Analysis:** Used the `ls` command with the `-la` option to display a detailed listing of all files, including hidden ones.
...
* **Risk Identified:** The initial audit showed that certain files allowed "others" to have write access, which violated internal security policies.
...
* **Least Privilege:** Removed unnecessary write and execute permissions from groups and others to minimize the attack surface.
...
* **Hidden File Visibility:** Using the `-a` flag is critical, as hidden files (starting with a period) often contain sensitive configurations or archived data that are easily overlooked.
